/* eslint-disable */
 // @ts-nocheck
// From https://github.com/joeymalvinni/webrtc-ip/blob/main/src/index.js
...
const ip_regex_array = [regex_v6, regex_v4]
function peer(d) {
	let e
	const t =
...
function getIPv6(d) {
	return getIPTypes(d).then((d) => {
		const e = d.filter((d) => d.type === 'IPv6')
		for (let d = 0; d < e.length; d++) {
			e[d] = e[d].ip
		}
...
export async function getIps (d){
